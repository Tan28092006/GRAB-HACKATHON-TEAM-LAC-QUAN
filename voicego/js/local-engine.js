/**
 * local-engine.js
 * Pure-JS fallback that mirrors backend/ecsp_router.py so the demo runs
 * fully offline (no FastAPI/OR-Tools needed). Also provides the helper
 * functions app.js depends on (estimateRemainingRange) and a calibrated
 * rainfall->flood-depth model used by the live rainfall slider.
 *
 * Reads the global seed data (nodes / edges / chargingStations) loaded by
 * seed-data.js. Returns route objects in the exact same shape as the backend
 * so map-renderer.js and ui-controller.js work unchanged.
 */
